...
callback2=function(response){top5_update('top5_data',response);}
ajax_hivas(SZERVER+'/app_ilekvar/code/top5.php?random='+Math.random()+"&myid="+myID+"&kedvenceim="+JSON.stringify(Kedvenceim)+"&analytics="+JSON.stringify(AN), 'callback2','top5_data');var element1=document.getElementById('RECEPT_MEZO1');var balra1=Hammer(element1).on("swipeleft", function(event){ReceptMenu(2);});var element2=document.getElementById('RECEPT_MEZO2');var balra2=Hammer(element2).on("swipeleft", function(event){ReceptMenu(3);});var jobbra2=Hammer(element2).on("swiperight", function(event){ReceptMenu(1);});var element3=document.getElementById('RECEPT_MEZO3');var balra3=Hammer(element3).on("swipeleft", function(event){ReceptMenu(4);});var jobbra3=Hammer(element3).on("swiperight", function(event){ReceptMenu(2);});var element4=document.getElementById('RECEPT_MEZO4');var jobbra4=Hammer(element4).on("swiperight", function(event){ReceptMenu(3);});Hammer(document.getElementById("KAT1")).on("tap",function(event){Kat(1);} );Hammer(document.getElementById("KAT2")).on("tap",function(event){Kat(2);} );Hammer(document.getElementById("KAT3")).on("tap",function(event){Kat(3);} );Hammer(document.getElementById("KAT4")).on("tap",function(event){Kat(4);} );Hammer(document.getElementById("KAT5")).on("tap",function(event){Kat(5);} );Hammer(document.getElementById("KAT6")).on("tap",function(event){Kat(6);} );Hammer(document.getElementById("ShareIkon2")).on("tap",function(event){Megoszt(0);} );Hammer(document.getElementById("FacebookIkon2")).on("tap",function(event){Megoszt(1);} );Hammer(document.getElementById("TwitterIkon2")).on("tap",function(event){Megoszt(2);} );document.getElementById("ADAG").value="4";document.getElementById("WIFI_ONLY").src="img/beki"+((Preferences(1)==0)?0:1)+".png";document.getElementById("WIFI_ONLY").className=((Preferences(1)==0)?"setting":"setting on");document.getElementById("KAT_SEARCH").src="img/beki"+((Preferences(2)==1)?1:0)+".png";document.getElementById("KAT_SEARCH").className=((Preferences(2)==0)?"setting":"setting on");for (var e=0;e<ElozmenyekMAX;e++)
...
document.addEventListener("backbutton", Vissza, false);var RNEV=document.getElementById("RNEV");RNEV.style.fontSize=RNevH+"px";RNEV.style.lineHeight=RNevLH+"px";var RIDO=document.getElementById("RIDO");RIDO.style.fontSize=RIdoH+"px";var RCSI=document.getElementById("RSIPKA");RCSI.style.height=parseInt(RCsiH)+"px";if (window.device)
{setTimeout(function(){navigator.splashscreen.hide();}, 6000);}
if (window.device){FB.init({appId: "542071045911013", nativeInterface: CDV.FB, useCachedDialogs: false});}
Ertekeles_letolt();for (var m=1;m<=7;m++)
{Sor(document.getElementById("POROND0"),null,null);}
Evszak_Init();document.getElementById("KAT6").src="img/evszakok/evszak"+EVSZAKnr+".png";var K=document.getElementById("KERES");K.parentNode.style.height=Szazalek * 8+"px";K.focus();K.select();document.getElementById("KERES_GOMB").style.height=Szazalek * 10+"px";document.getElementById("IDOVONAL1").style.height=Szazalek * 22+"px";document.getElementById("IDOVONAL2").style.height=Szazalek * 22+"px";document.getElementById("SKALA1").style.height=Szazalek * 6+"px";document.getElementById("SKALA2").style.height=Szazalek * 6+"px";setTimeout(function(){console.log("--- Reklam indul");ReklamOFF();},500);if (window.device){setTimeout(function(){navigator.splashscreen.hide();console.log("SplashScreen KIKAPCS");}, 3500);}}};var KategoriaID=Array();var KategoriaNEV=Array();var Receptek=Array();var LastPage=[1];var LastLablec=[1];var LastRecept=[0];var LastKategoria=[];var LastKat=0;function Oldal(oldal,lablec)
...
{var CSIK=document.getElementById("CSIK6")
if (KAT) 
{CSIK.src="img/fejlec6"+Math.abs(KAT)+".jpg";}
else
{CSIK.src="img/fejlec2t.jpg";}
var PS=document.getElementById("SCROLLER"+oldal);if (PS){PS.innerHTML="<div id='POROND_TERKOZ"+oldal+"' class='porond_terkoz' style='height:"+parseInt(window.innerHeight*0.0001)+"px'></div>";}
...
window.localStorage.setItem("E"+ReceptMost,NUM);callback=function(response){Atlag_update('ATLAG',response);}
ajax_hivas(SZERVER+'/app_ilekvar/code/ertekeles.php?random='+Math.random()+MODE, 'callback','ATLAG');}
...
if (window.device && !network_status(true))
{document.getElementById("NotificationMessage").innerHTML="Az adatok frissítéséhez internetkapcsolatra van szükség.";document.getElementById("Notification").setAttribute("onclick","this.style.display='none';");document.getElementById("Notification").style.display="block";}
